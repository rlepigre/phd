...
let biblio =
  ...
  ; notes   = "Annals of Mathematical Studies, Volume 6"
  ; year    = "1941"
  ; url     = "" }; (* FIXME introuvable *)

  { key     = "Barendregt1981"
  ; authors = ["Hendrik P. Barendregt"]
  ...
  ; title   = "Interprétation fonctionnelle et élimination des coupures "
              ++ "de l'arithmétique d'ordre supér-ieur"
  ; notes   = "Thèse de l'Université Paris VII"
  ; year    = "1972"
  ; url     = "" }; (* FIXME introuvable *)

  { key     = "Girard1989"
  ; authors = ["Jean-Yves Girard"; "Paul Taylor"; "Yves Lafont"]
  ...
  { key     = "Damas1982"
  ; authors = ["Luís Damas"; "Robin Milner"]
  ...
  { key     = "Harper1991"
  ; authors = ["Robert Harper"; "Mark Lillibridge"]
  ...
  { key     = "Constable1986"
  ; authors = ["Robert. L. Constable"; "S. F. Allen"; "H. M. Bromley"
              ; "W. R. Cleaveland"; "J. F Cremer"; "Robert Harper"
              ; "D. J. Howe"; "T. B. Knoblock"; "N. P. Mendler"
  ...
  { key     = "Owre1996"
  ; authors = ["S. Owre"; "S. Rajan"; "J. Rushby"; "N. Shankar"; "M. Srivas"]
  ...
  ; title   = "A Classical Realizability Model for a Semantical Value "
              ++ "Restriction"
  ; notes   = "proceedings of ESOP, Lecture Notes in Computer Science"
              ++ ", Volume 9632"
  ; year    = "2016"
  ; url     = "https://dx.doi.org/10.1007/978-3-662-49498-1_19" };

  { key     = "Church1936"
  ; authors = ["Alonzo Church"; "John Barkley Rosser Sr."]
...
(* https://www.cs.cmu.edu/~joshuad/papers/letnormal/Dunfield10_letnormal.pdf *)
  ...
  { key     = "Flanagan1993"
  ; authors = ["Cormac Flanagan"; "Amr Sabry"; "Bruce Duba"
              ; "Matthias Felleisen" ]
  ...
  { key     = "Tarditi1996"
  ; authors = ["D. Tarditi"; "G. Morrisett"; "P. Cheng"
              ; "C. Stone"; "R. Harper"; "P. Lee"]
  ...
  { key     = "Chlipala2005"
  ; authors = ["Adam Chlipala"; "Leaf Petersen"; "Robert Harper"]
  ...
  { key     = "Mitchell1991"
  ; authors = ["L. Cardelli"; "S. Martini"; "J. C. Mitchell"; "A. Scedrov"]
  ; title   = "An Extension of System F with Subtyping"
  ; notes   = "proceedings of TACS"
  ; year    = "1991"
  ...
  { key     = "Hughes1996"
  ; authors = ["John Hughes"; "Lars Pareto"; "Amr Sabry"]
  ...
  ; notes   = "SIGPLAN Notices 38(9)"
  ; year    = "2003"
  ; url     = "https://doi.acm.org/10.1145/944746.944723" };

  { key     = "Manoury1992"
  ; authors = ["Pascal Manoury"; "Michel Parigot"; "Marianne Simonot"]
  ...
  ; notes   = "Thèse de l'Université Paris Diderot"
  ; year    = "2003"
